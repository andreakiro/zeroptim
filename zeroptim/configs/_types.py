...
class ModelConfig(BaseModel):
    ...
    model_type: str
    model_hparams: Dict[str, Any] # underscore to avoid conflict with Pydantic BaseModel
    ...
    def check_sub_optimizer(cls, values):
        ...
        if optimizer_type in ["mezo", "smartes"]:
            ...
            if not sub_optimizer_type:
                raise ValueError(f"'sub_optimizer_type' must be specified for optimizer_type '{optimizer_type}'")
            if sub_optimizer_type in ["mezo", "smartes"]:
                raise ValueError(f"'sub_optimizer_type' cannot be '{sub_optimizer_type}' for optimizer_type '{optimizer_type}'")
        
            if sub_optimizer_type and sub_optimizer_type not in __supported_optims__:
                raise ValueError(f"'sub_optimizer_type' {sub_optimizer_type} is not a supported optimizer")

...
class SmartES(torch.optim.Optimizer):
    ...
    def __init__(self, opt, model, eps=0.01):
        ...
        super().__init__(
            self.opt.param_groups, {"eps": self.eps, "eps_var": self.eps_sigma}
        )
    ...
    def step(self, closure=None):
        ...
        try:
            ...
            raise RuntimeError(
                str(e) + ". Hint: ensure that backward is disbaled inside closure."
            )
            ...
            if isinstance(module, torch.nn.Linear):
                rm_hooks.append(
                    module.register_forward_hook(
                        self.build_hook(proj_grad, update_param=True)
                    )
                )
